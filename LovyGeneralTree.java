...
//
    private static <AnyType> LovyNode<AnyType> createLovyNodeFromLovyGeneralNode(LovyGeneralNode<AnyType> lovyGeneralNode){
    ...
    public static <AnyType> LovyTree<AnyType> createLovyTreeFromLovyForest(LovyGeneralTree<AnyType>[] forest){
        if(forest==null||forest.length==0)
            return null;
        LovyTree<AnyType> root=new LovyTree<AnyType>(createLovyNodeFromLovyGeneralNode(forest[0].tree[0]));
        LovyNode<AnyType> p=root.getRoot();
        for(int i=1;i<forest.length;i++){
            p.setRight(createLovyNodeFromLovyGeneralNode(forest[i].tree[0]));
            p=p.getRight();
        }
        return root;
    }
    ...
    public static void main(String[] args)throws Exception{
        ...
        for(int i=0;i<3;i++){
            LovyGeneralTree lovyGeneralTree=createLovyGeneralTree(LovyGeneralParentPointTree.getTestData("d:/tree"+(i+1)+".lvy"));
